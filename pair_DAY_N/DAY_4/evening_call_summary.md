...
The asker also wanted the recommendation connected more directly to ablation reliability. The revised version now explains that Delta A/B can be inflated if the trained method is consistently placed in the favoured position, and recommends reporting unstable-pair rate, position-bias flags, and stable-win-only deltas.
